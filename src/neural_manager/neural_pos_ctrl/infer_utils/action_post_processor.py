...
class ActionPostProcessor:
    ...
    def __init__(
        ...
        # Log control mode
        if self._logger:
            mode_name = "Throttle + Body Rates (VehicleRatesSetpoint)" if self._control_mode == "throttle_rates" else "Thrust Acceleration + Body Rates (VehicleThrustAccSetpoint)"
    ...
    def print_control_command(self, action_display_dict: dict, prefix: str = ""):
        ...
        if control_mode == "throttle_rates":
            ...
            print(f"{prefix}  ⬆️  油门:           {action_display_dict['throttle']:.3f} (归一化, 原始: {action_display_dict['throttle_raw']:.3f})")
